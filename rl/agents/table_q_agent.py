...
class TableQAgent(abstract_agent.Agent):
    ...
    def train(self, obs, reward):
        if self.last_obs is not None:
            ...
            # ------------
            self.q_table[last_obs_key][self.last_action] = \
                (1 - self.learning_rate) * self.q_table[last_obs_key][self.last_action] \
    ...
    def select_action(self, obs, mode: str):
    ...
    def greedy(self, obs_key):
        action = np.argmax(self.q_table[obs_key])
        return action
